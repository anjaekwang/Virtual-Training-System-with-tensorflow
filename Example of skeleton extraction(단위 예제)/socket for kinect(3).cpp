...
//Winsock2의 bind 와 std의 bind 때문에 namespacer를 사용안함
using namespace cv;
...
void drawSkeleton(SOCKET sock, const NUI_SKELETON_DATA &position, IplImage* Skeleton)
{
	...
	// csv 저장(2) 하는 방식에서 한프레임의 좌표를 문자열 형식으로 열거해 바로 소켓을 보냄.
...
int main(int argc, char *argv[]) {
	...
	if (bind(sock, (SOCKADDR*)&sockinfo, sizeof(sockinfo)) == SOCKET_ERROR) //bind한 상태
		printf(" bind 실패 ");

	if (listen(sock, 5) == SOCKET_ERROR)
		printf(" 대기열 실패 ");    //커널에 서버소켓 등록
